...
class ListingCreate(BaseModel):
    ...
    furnished: str = Field(pattern=r"^(furnished|semi_furnished|unfurnished)$")
...
class ListingFilter(BaseModel):
    ...
    def validate_ranges(self):
        ...
        if self.furnished is not None and self.furnished not in {"furnished", "semi_furnished", "unfurnished"}:
            raise ValueError("furnished must be one of furnished, semi_furnished, unfurnished")
        if self.sort_by is not None and self.sort_by not in {"price", "size", "bedrooms", "bathrooms", "created", "title"}:
            raise ValueError("sort_by must be one of price, size, bedrooms, bathrooms, created, title")
