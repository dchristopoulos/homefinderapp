...
from app.services.viewing_service import my_viewings_use_case
...
def _send_password_reset_email(email: str, reset_token: str) -> None:
    ...
    send_email(
        EmailMessage(
            to=email,
            subject="Reset your HomeFinder password",
            body=(
                "We received a request to reset your HomeFinder password.\n\n"
...
def account_settings(request: Request, db: Session = Depends(get_db), message: str | None = None):
    ...
    viewings = my_viewings_use_case(db, user_id=int(user.id))
    reservations = my_reservations_use_case(db, user_id=int(user.id))
    return template_response(
        request,
        "account.html",
        {
...
def update_own_profile(
    ...
    if email_changed:
        ...
        background_tasks.add_task(send_verification_email, str(updated.email))
        return RedirectResponse(
