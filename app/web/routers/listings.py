...
from app.core.exceptions import FavoriteNotFoundError, InvalidDateRangeError, ListingNotFoundError, OwnerNotFoundError, PermissionDeniedError, ReservationConflictError, ViewingConflictError
...
from app.services.viewing_service import create_viewing_use_case
from app.schemas.viewing import ViewingCreate
from app.services.reservation_service import create_reservation_use_case
...
def _normalize_scores(values: list[float], invert: bool = False) -> list[float]:
    ...
    if invert:
        best = min(positive_values)
        return [100.0 if value <= 0 else max(0.0, min(100.0, 100.0 * best / value)) for value in values]

    best = max(positive_values)
    return [max(0.0, min(100.0, 100.0 * value / best)) for value in values]
...
def create_listing_submit(
    ...
    if image is None or not image.filename:
        return template_response(
            request,
            "create_listing.html",
            {"error": "A primary listing photo is required."},
            status_code=400,
        )
    try:
        ...
        if getattr(user, "role", "") == "regular_user":
            change_user_role(
                db,
        ...
        return template_response(request, "create_listing.html", {"error": "Owner not found"}, status_code=400)
...
def listing_detail(
    ...
    try:
        ...
        raise HTTPException(status_code=404, detail="Listing not found") from exc
    owner = get_user_profile(db, listing.owner_id)
    current_user_id, saved_listing_ids = _saved_listing_state(request, db)
    return template_response(
        request,
        "listing_detail.html",
...
@router.post("/listings/{listing_id}/contact-owner")
def contact_owner_submit(
    request: Request,
    listing_id: int,
    csrf_token: str | None = Form(default=None),
    ...
    success_text = quote("Inquiry sent to the property owner.")
    return RedirectResponse(
...
@router.post("/listings/{listing_id}/schedule-viewing")
def schedule_viewing_submit(
    request: Request,
    listing_id: int,
    csrf_token: str | None = Form(default=None),
    ...
    try:
        ...
        error_text = quote("Please enter a valid date and time.")
        return RedirectResponse(url=f"/listings/{listing_id}?error={error_text}", status_code=303)
    ...
    try:
        ...
        create_viewing_use_case(db, user_id=current_user_id, payload=payload)
    except ViewingConflictError:
        error_text = quote("That time slot is already booked. Please choose a different time.")
        return RedirectResponse(url=f"/listings/{listing_id}?error={error_text}", status_code=303)
    except ListingNotFoundError as exc:
        raise HTTPException(status_code=404, detail="Listing not found") from exc
    except Exception:
        error_text = quote("Could not schedule the viewing. Please check your date and try again.")
        return RedirectResponse(url=f"/listings/{listing_id}?error={error_text}", status_code=303)

    success_text = quote("Viewing scheduled! You will receive a confirmation email.")
    return RedirectResponse(url=f"/listings/{listing_id}?message={success_text}", status_code=303)


@router.post("/listings/{listing_id}/reserve")
def reserve_listing_submit(
    request: Request,
    listing_id: int,
    csrf_token: str | None = Form(default=None),
    ...
    try:
        ...
        error_text = quote("Please enter valid start and end dates.")
        return RedirectResponse(url=f"/listings/{listing_id}?error={error_text}", status_code=303)
    ...
    try:
        ...
        create_reservation_use_case(db, user_id=current_user_id, payload=payload)
    except InvalidDateRangeError:
        error_text = quote("End date must be on or after the start date.")
        return RedirectResponse(url=f"/listings/{listing_id}?error={error_text}", status_code=303)
    except ReservationConflictError:
        error_text = quote("Those dates are already reserved. Please choose different dates.")
        return RedirectResponse(url=f"/listings/{listing_id}?error={error_text}", status_code=303)
    except PermissionDeniedError:
        error_text = quote("You cannot reserve your own listing.")
        return RedirectResponse(url=f"/listings/{listing_id}?error={error_text}", status_code=303)
    except ListingNotFoundError as exc:
        raise HTTPException(status_code=404, detail="Listing not found") from exc
    except Exception:
        error_text = quote("Could not create the reservation. Please try again.")
        return RedirectResponse(url=f"/listings/{listing_id}?error={error_text}", status_code=303)

    success_text = quote("Reservation submitted! You will receive a confirmation email.")
    return RedirectResponse(url=f"/listings/{listing_id}?message={success_text}", status_code=303)
