...
router = APIRouter()
logger = logging.getLogger("homefinder.web.messages")
...
def messages_home(request: Request, db: Session = Depends(get_db)):
    ...
    if str(user.role) in {"property_owner", "admin"}:
...
def my_messages_page(
    ...
    rows = [{"inquiry": inquiry, "listing": listing, "owner": owner} for inquiry, listing, owner in raw_rows]
    ...
    return template_response(
        ...
        "messages_my.html",
        {
            ...
            "can_view_inbox": str(user.role) in {"property_owner", "admin"},
...
def owner_inbox_page(
    ...
    rows = [
        ...
        for inquiry, listing, sender, owner in raw_rows
    ]
    inquiry_ids = [int(item["inquiry"].id) for item in rows]
    thread_rows = replies_for_inquiry_ids_use_case(db, inquiry_ids=inquiry_ids, per_inquiry_limit=12)
    threads_by_inquiry: dict[int, list[dict[str, str | int | None]]] = {}
    ...
    owned_count = 1 if is_admin_inbox else int(db.query(func.count(Listing.id)).filter(Listing.owner_id == int(user.id)).scalar() or 0)
    return template_response(
        request,
        "messages_inbox.html",
        {
...
def message_contact_profile_page(
    ...
    if int(contact.id) != int(user.id) and str(user.role) != "admin":
        has_shared_inquiry = (
            ...
            .filter(
                or_(
                    and_(Inquiry.user_id == int(user.id), Listing.owner_id == int(contact.id)),
                    and_(Inquiry.user_id == int(contact.id), Listing.owner_id == int(user.id)),
                )
            )
            .first()
            is not None
    ...
    listing_count = db.query(Listing.id).filter(Listing.owner_id == int(contact.id)).count()
    sent_inquiries_count = db.query(Inquiry.id).filter(Inquiry.user_id == int(contact.id)).count()
    received_inquiries_count = (
        db.query(Inquiry.id).join(Listing, Listing.id == Inquiry.listing_id).filter(Listing.owner_id == int(contact.id)).count()
    )
    ...
    if str(user.role) == "admin":
        shared_inquiries_query = (
            db.query(Inquiry)
            .join(Listing, Listing.id == Inquiry.listing_id)
            .filter((Inquiry.user_id == int(contact.id)) | (Listing.owner_id == int(contact.id)))
            .order_by(Inquiry.created_at.desc())
        )
