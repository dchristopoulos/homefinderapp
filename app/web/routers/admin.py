...
def admin_dashboard(request: Request, db: Session = Depends(get_db), message: str | None = None):
    ...
    listing_count_by_owner = Counter(item.owner_id for item in listings)
    ...
    user_stats = [
        {
            ...
            "listing_count": listing_count_by_owner.get(profile.id, 0),
    ...
    owner_performance: list[dict[str, int | str]] = []
    for owner_id, listing_count in listing_count_by_owner.items():
        owner = user_lookup.get(owner_id)
        if owner is None:
            continue
        owner_listing_ids = [listing.id for listing in listings if listing.owner_id == owner_id]
        owner_inquiries = sum(inquiries_by_listing.get(listing_id, 0) for listing_id in owner_listing_ids)
        owner_favorites = sum(favorites_by_listing.get(listing_id, 0) for listing_id in owner_listing_ids)
        owner_performance.append(
            {
                "username": str(owner.username),
                "listing_count": listing_count,
                "inquiries": owner_inquiries,
                "favorites": owner_favorites,
                "demand_score": owner_inquiries * 2 + owner_favorites,
            }
        )
    owner_performance = sorted(owner_performance, key=lambda item: int(item["demand_score"]), reverse=True)[:8]
    ...
    admin_overview = {
        ...
        "owner_accounts": role_counts.get("property_owner", 0),
    }
...
def admin_update_listing(
    ...
    if (
        ...
        or bedrooms < 0
        or bathrooms <= 0
        or furnished not in {"furnished", "semi_furnished", "unfurnished"}
    ):
