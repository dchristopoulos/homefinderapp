...
def dashboard(request: Request, db: Session = Depends(get_db), message: str | None = None):
    ...
    onboarding = {
        "has_listing": any(item.owner_id == user.id for item in listings),
        "has_favorite": any(item.user_id == user.id for item in favorites),
        "has_inquiry": any(item.user_id == user.id for item in inquiries),
    }

    my_listings = [item for item in listings if item.owner_id == user.id]
        ...
        quick_actions.append(
            {
                ...
                "detail": "Start conversations to unlock viewings faster.",
                "href": "/listings",
            }
        )
    ...
    stats = {
        ...
        "role_breakdown": {
            "regular_user": role_counts.get("regular_user", 0),
            "property_owner": role_counts.get("property_owner", 0),
            "service_supervisor": role_counts.get("service_supervisor", 0),
