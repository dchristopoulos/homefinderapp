...
router = APIRouter()
logger = logging.getLogger("homefinder.web.auth")
...
def register_user_submit(
    ...
    if is_rate_limited(db, request, "register_web", email, settings.register_rate_limit_per_minute):
...
def resend_verification_email(
    ...
    if account is not None and not is_email_verified(account):
        background_tasks.add_task(send_verification_email, str(account.email))
