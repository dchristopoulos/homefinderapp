...
from app.core.database_guard import describe_database_url, validate_homefinder_database_url
...
async def lifespan(app: FastAPI):
    configure_logging()
    _initialize_sentry()
    db_path = validate_homefinder_database_url(
        settings.database_url,
        project_root=PROJECT_ROOT,
        app_env=settings.app_env,
    )
    logger.info(
        "HomeFinder database target resolved",
        extra={
            "database_target": describe_database_url(settings.database_url, project_root=PROJECT_ROOT),
            "database_file": str(db_path) if db_path is not None else None,
        },
    )
    init_db()
    worker_task = None
