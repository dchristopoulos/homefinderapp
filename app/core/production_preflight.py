...
from app.core.database_guard import validate_homefinder_database_url
...
def evaluate_production_env(env: Mapping[str, str]) -> tuple[list[str], list[str]]:
    ...
    database_url_raw = env.get("DATABASE_URL", "").strip()
    ...
    try:
        validate_homefinder_database_url(database_url_raw, project_root=PROJECT_ROOT, app_env=app_env)
    ...
    smtp_use_tls = _is_truthy(env.get("SMTP_USE_TLS", "false"))
    smtp_start_tls = _is_truthy(env.get("SMTP_START_TLS", "true"))
    if smtp_use_tls and smtp_start_tls:
        errors.append("SMTP_USE_TLS and SMTP_START_TLS cannot both be enabled.")
    if smtp_enabled:
    ...
    try:
        ...
        register_rate = _get_int(env, "REGISTER_RATE_LIMIT_PER_MINUTE", 8)
        if login_rate <= 0 or api_rate <= 0 or register_rate <= 0:
            errors.append("Rate limits must be positive integers.")
    except ValueError:
        errors.append(
            "API_RATE_LIMIT_PER_MINUTE, LOGIN_RATE_LIMIT_PER_MINUTE, "
            "and REGISTER_RATE_LIMIT_PER_MINUTE must be integers."
        )
