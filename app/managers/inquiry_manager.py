...
def list_inquiry_rows_for_user(
    ...
    *,
    ...
    query = (
        ...
        .outerjoin(User, User.id == Listing.owner_id)
...
def list_inquiry_rows_for_inbox(
    ...
    *,
    ...
    query = (
        db.query(Inquiry, Listing, sender_alias, owner_alias)
        .outerjoin(Listing, Listing.id == Inquiry.listing_id)
        .outerjoin(sender_alias, sender_alias.id == Inquiry.user_id)
        .outerjoin(owner_alias, owner_alias.id == Listing.owner_id)
    )
    if actor_role != "admin":
        query = query.filter(Listing.owner_id == actor_user_id)
    if status:
        query = query.filter(Inquiry.status == status)
    if search:
        like = f"%{search}%"
        query = query.filter(
            or_(
                ...
                owner_alias.username.ilike(like),
            )
        )
