...
from app.core.exceptions import InvalidFilterError, OwnerNotFoundError, PermissionDeniedError
...
def create_listing_endpoint(
    ...
    if current_user.id != listing_in.owner_id and not can_manage_all:
        raise PermissionDeniedError

    owner = get_user_by_id(db, listing_in.owner_id)
    if owner is None:
        raise OwnerNotFoundError
    if not has_permission(
        owner.role,
        "manage_own_listings",
        owner.permission_grants,
        owner.permission_revokes,
    ):
        raise PermissionDeniedError
