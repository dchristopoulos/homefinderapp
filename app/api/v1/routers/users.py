...
class LoginVerifyRequest(BaseModel):
    ...
    @property
    def supplied_identifier(self) -> str | None:
...
class RoleUpdateRequest(BaseModel):
    role: str = Field(pattern=r"^(regular_user|property_owner|service_supervisor|admin)$")
...
def create_user_endpoint(user_in: UserCreate, background_tasks: BackgroundTasks, db: Session = Depends(get_db)):
    if user_in.role != "regular_user":
...
def login_init_endpoint(payload: LoginInitRequest, db: Session = Depends(get_db)):
    user = get_user_by_identifier_use_case(db, payload.login_identifier)
    if user is None:
        raise UserNotFoundError
...
def login_verify_endpoint(payload: LoginVerifyRequest, db: Session = Depends(get_db)):
    ...
    if identifier is None:
        raise UserNotFoundError
    supplied_identifier = payload.supplied_identifier
