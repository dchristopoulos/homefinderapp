...
logger = logging.getLogger("homefinder.reservations")
...
def create_reservation_use_case(db: Session, *, user_id: int, payload: ReservationCreate) -> Reservation:
    ...
    if user_id == int(cast(Any, listing.owner_id)):
...
def listing_reservations_use_case(
    ...
    *,
    ...
    if listing is None:
        raise ListingNotFoundError
    if actor_role != "admin" and actor_user_id != int(cast(Any, listing.owner_id)):
        raise PermissionDeniedError
