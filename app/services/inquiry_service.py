...
logger = logging.getLogger("homefinder.inquiries")
...
def listing_inquiries_use_case(db: Session, *, listing_id: int, actor_user_id: int, actor_role: str) -> list[Inquiry]:
    ...
    if listing is None:
        raise ListingNotFoundError
    listing_owner_id = int(cast(Any, listing.owner_id))
    if actor_role != "admin" and actor_user_id != listing_owner_id:
...
def _is_inquiry_participant(
    *,
    ...
    if actor_user_id == int(cast(Any, inquiry.user_id)):
        return True
    if actor_user_id == int(listing_owner_id):
        return True
    return False
